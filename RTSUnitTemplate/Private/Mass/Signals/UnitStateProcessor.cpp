...
void UUnitStateProcessor::UnbindDelegates_Internal()
{
	...
	if (SignalSubsystem)
	{
		...
		if (LoadUnitDelegateHandle.IsValid())
		{
	...
	AWorkArea* BuildArea)
{
	...
	{
		// Wenn kein GroundTrace gemacht wird, ist es wahrscheinlich eine Extension in der Luft
		if (BuildArea && BuildArea->IsExtensionArea)
		{
			// Wir berechnen die FlyHeight fuer den Client (Abstand zum Boden)
			FHitResult HitResult;
			FCollisionObjectQueryParams ObjectParams;
			ObjectParams.AddObjectTypesToQuery(ECC_WorldStatic);
			FCollisionQueryParams TraceParams(FName(TEXT("UnitFlyHeightTrace")), true, UnitBase);
			TraceParams.AddIgnoredActor(UnitBase);
            
			// Trace weit nach unten
			if (GetWorld()->LineTraceSingleByObjectType(HitResult, Location, Location - FVector(0, 0, 20000), ObjectParams, TraceParams))
			{
				UnitBase->FlyHeight = Location.Z - HitResult.ImpactPoint.Z;
			}
			else
			{
				UnitBase->FlyHeight = 0.f;
			}
		}
	}
