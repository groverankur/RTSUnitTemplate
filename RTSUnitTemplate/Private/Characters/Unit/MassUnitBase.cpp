...
AMassUnitBase::AMassUnitBase(const FObjectInitializer& ObjectInitializer)
{
	...
	if (MassActorBindingComponent)
	{
		...
		// The animation processor writes custom-data indices 1..12 (13 floats) and the corpse-dissolve
		// writes index 13, so 14 total. Size the template here so it is correct before any instance is
		// added, and so the value copied into the pooled ISM in UUnitVisualManager::AssignUnitVisual is
		// never undersized. No instances exist at construction, so this just sets the default count.
		ISMComponent->NumCustomDataFloats = 14;
	}
...
bool AMassUnitBase::GetMassEntityData(FMassEntityManager*& OutEntityManager, FMassEntityHandle& OutEntityHandle)
{
	...
	if (!OutEntityHandle.IsSet())
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("AMassUnitBase (%s): Cannot get Mass Entity Data - Entity Handle is not set in Binding Component."), *GetName());
		return false;
	}
